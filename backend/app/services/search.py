...
def normalize_query(q: Dict[str, Any]) -> Dict[str, Any]:
    keys = ["full_name", "dob", "aliases", "last_known_locations", "location", "relatives", "local_only"]
    out: Dict[str, Any] = {}
    for k in keys:
        v = q.get(k)
        if isinstance(v, str):
...
def build_linkouts(q: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    linkouts.extend([
        ...
        },
        {
            ...
            "category": "social_media",
            "priority": 3,
            "url": _gsearch(f'"{name}" {last} site:facebook.com OR site:linkedin.com OR site:twitter.com'),
...
def set_cache(qh: str, q: Dict[str, Any], results: List[Dict[str, Any]]) -> Dict[str, str]:
    created_at = _now_iso()
    expires_at = (datetime.utcnow() + timedelta(days=DEFAULT_CACHE_DAYS)).replace(microsecond=0).isoformat() + "Z"
    with db_cursor() as cur:
        cur.execute(
