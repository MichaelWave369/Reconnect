...
def _fts_available(cur) -> bool:
    try:
        row = cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='fts_all'").fetchone()
...
def rebuild_fts(case_id: Optional[str] = None) -> None:
    ...
    with db_cursor() as cur:
        ...
        for r in cur.execute(q_cases, params).fetchall():
            body = " ".join([r["subject_name"] or "", r["aliases"] or "", r["last_known_locations"] or "", r["relatives"] or "", r["notes"] or ""])
            cur.execute("INSERT INTO fts_all(case_id, ref_type, ref_id, title, body) VALUES (?,?,?,?,?)",
...
def global_search(q: str, case_id: Optional[str] = None, limit: int = 30) -> List[Dict[str, Any]]:
    ...
    with db_cursor() as cur:
        ...
        if use_fts:
            ...
            return [{"case_id": r["case_id"], "ref_type": r["ref_type"], "ref_id": r["ref_id"], "title": r["title"], "snippet": r["snippet"]} for r in rows]
        ...
        def add(rows, ref_type: str, title_key="title", body_key="body"):
            for r in rows:
                out.append({
                    ...
                    "snippet": (r.get(body_key) if isinstance(r, dict) else r[body_key])[:160] if (r.get(body_key) if isinstance(r, dict) else r[body_key]) else ""
                })
        ...
        out.extend([{"case_id": r["case_id"], "ref_type": "case", "ref_id": r["id"], "title": r["title"], "snippet": (r["body"] or "")[:160]} for r in case_rows])
        ...
        out.extend([{"case_id": r["case_id"], "ref_type": "evidence", "ref_id": r["id"], "title": r["title"], "snippet": (r["body"] or "")[:160]} for r in ev_rows])
        ...
        out.extend([{"case_id": r["case_id"], "ref_type": "task", "ref_id": r["id"], "title": r["title"], "snippet": (r["body"] or "")[:160]} for r in t_rows])
        ...
        out.extend([{"case_id": r["case_id"], "ref_type": "doc", "ref_id": r["id"], "title": r["title"], "snippet": (r["body"] or "")[:160]} for r in d_rows])
